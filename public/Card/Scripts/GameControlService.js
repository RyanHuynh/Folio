app.service('GameControlService', function($http, $timeout, GameStateService, StatService){
	 ...
	 ********************************/
	 ...
	 //Lock creating new game to avoid inconsistent. 
	 ...
	 //Handle display text on display screen.
	...
	//Load game with delay to avoid incosistent
	this.loadGame = function(){
		_disableStatBtn(true);
		_gameLocked = true;
	...
	//Disable stat button when timer is activate
	var _disableStatBtn = function(flag){
		_statBtnDisabled = flag;
	}
	//Check to see if stat button is disabled.
	this.isStatDisabled = function(){
		return _statBtnDisabled;
	}

	//Kill timer timeout function.
